...
export function HolographicHero() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!gl) {
        console.warn('[HolographicHero] WebGL not supported, skipping 3D background');
        setHasError(true);
        return;
      }
      ...
      console.error('[HolographicHero] Initialization error:', error);
      setHasError(true);
    }
  }, []);

  // Render nothing if WebGL is not supported or initialization failed
  if (hasError) {
    return null;
  }
